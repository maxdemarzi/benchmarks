...
class BusinessIntelligence5 extends Simulation {
  ...
  val statements = """{"statements" : [{"statement" : "%s", "parameters" : { "country": "Belarus" } }] }"""
    .format(query)

  val scn = scenario("theirs.BusinessIntelligence5")
    .during(30 ) {
        exec(
          http("BI-5 (theirs)")
